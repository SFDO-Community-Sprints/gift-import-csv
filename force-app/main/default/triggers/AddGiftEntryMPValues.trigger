trigger AddGiftEntryMPValues on GiftEntry (before insert, before update) {

    for (GiftEntry rec : Trigger.new) {

        if (rec.FirstName != Trigger.oldMap.get(rec.id).FirstName || rec.LastName != Trigger.oldMap.get(rec.id).LastName) {
